<nav class="sidebar sidebar-offcanvas" id="sidebar" wire:ignore>
    <ul class="nav">
      ...
      @if (count($item['childern'])>0 && Auth::guard('dokter')->user()->hasAnyPermission(explode('|',$item['permission'])))
      <li class="nav-item">
        ...
        <div class="collapse" id="{{ Str::slug($item['title']) }}">
            <ul class="nav flex-column sub-menu">
                @foreach ($item['childern'] as $child)
                    @if (Auth::guard('dokter')->user()->can($child['permission']))
        ...
        @else
            @if (Auth::guard('dokter')->user()->can($item['permission']))
                <li class="nav-item">
