...
<div class="card">
        <div class="card-body">
            ...
            <form class="forms-sample" wire:submit.prevent="save">
                ...
                <x-select title="Pasien" wireModel='resep_n_diagnosa.pasien_id' :options="$pasienOptions" addAttributes="required"/>
                <x-select title="Dokter" wireModel='resep_n_diagnosa.dokter_id' :options="$dokterOptions" addAttributes="required"/>
                <x-cms.tiny-mce labelName="Resep" readonly="0" wire:model="resep_n_diagnosa.resep" note="note:optional" placeholder="Type anything you want..."/>
                @error('resep_n_diagnosa.resep')
                    <div class="font-size-sm mt-2 text-danger">
                        <span class="text-danger error">{{ $message }}</span>
                    </div>
                @enderror
                <x-cms.tiny-mce labelName="Diagnosa" readonly="0" wire:model="resep_n_diagnosa.diagnosa" note="note:optional" placeholder="Type anything you want..."/>
