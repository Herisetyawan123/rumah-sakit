...
<div class="card">
    <div class="card-body">
        <h4 class="card-title">Show Riwayat Konsultasi #{{ $konsultasi->id }}</h4>
        ...
        <form class="forms-sample" wire:submit.prevent="save">
            ...
            <x-select title="Status Konsultasi" wireModel='konsultasi.status_konsultasi' :options="$statusKonsultasiOptions" placeholder="Status Konsultasi" addAttributes="disabled"/>
            ...
            @can('cms.konsultasi.update')
                <a href="{{ route('cms.konsultasi.edit',['konsultasi'=>$konsultasi->id]) }}" class="btn btn-warning mr-2" >Edit</a>
            @endcan
            <a type="button" class="btn btn-outline-danger" href="{{ route('cms.konsultasi.riwayat') }}">Cancel</a>
        </form>
        </div>
    </div>
</div>
