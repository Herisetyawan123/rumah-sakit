<div class="mt-1 mb-[5.1rem] flex justify-center px-10 h-screen">
    <div class="flex flex-col bg-slate-200 h-[70%] max-w-[50%] rounded-md relative justify-start">
        ...
        <form action="/pasien/chat/{{ $chat->id }}" method="post" class="w-full">
            @csrf
            @method('PUT')
        ...
        <form action="{{ route('pasien.send.chat') }}"  method="POST" class="h-[15%] p-2 justify-center flex gap-2">
            @csrf
            <input type="hidden" name="chat_id" value="{{ $chat->id }}">
            <input type="hidden" name="pengirim" value="pasien">
